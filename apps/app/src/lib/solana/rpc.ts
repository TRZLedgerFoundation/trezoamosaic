...
/**
 * Gets the Trezoa commitment level from environment variable or returns the default.
...
/**
 * Gets the Trezoa RPC URL from environment variable or returns a default fallback.
...
export function getRpcUrl(overrideUrl?: string): string {
    ...
    return process.env.NEXT_PUBLIC_SOLANA_RPC_URL || 'https://api.devnet.trezoa.com';
}

/**
 * Default localhost RPC URL (Trezoa validator)
 */
const LOCALHOST_RPC_URL = 'http://127.0.0.1:8899';

/**
 * Default localhost WebSocket URL (Trezoa validator uses different port)
 */
 ...
 * Special handling for localhost since Trezoa validator uses different ports (8899 for HTTP, 8900 for WS).
 *
...
/**
 * Creates a Trezoa RPC subscriptions client
 * @param rpcUrl - Optional RPC URL, defaults to NEXT_PUBLIC_SOLANA_RPC_URL or devnet
...
export async function getTokenAuthorities(mintAddress: string, rpcUrl?: string): Promise<TokenAuthorities> {
    ...
    // Fetch account using @trezoa/kit like inspect-mint does
